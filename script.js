...
const DEFAULT_TAGLINE = "Helping the world work";

// Per-company memory, so each brand can carry its own address/tagline as you switch tabs.
const addressByCompany = {};
const taglineByCompany = {};
COMPANIES.forEach((c) => {
  addressByCompany[c.id] = DEFAULT_ADDRESS;
  taglineByCompany[c.id] = DEFAULT_TAGLINE;
});
...
function selectCompany(id) {
  ...
  taglineByCompany[activeCompanyId] = companyTaglineEl.value;

  activeCompanyId = id;
  const company = getActiveCompany();

  companyAddressEl.value = addressByCompany[id];
  companyTaglineEl.value = taglineByCompany[id];
...
function mainSentence(ctx) {
  ...
  if (engagement === "c2c") {
    return `This letter is to confirm that ${employee} is employed by ${company.name} through our staffing partner, ${staffingPartner || "[Staffing Partner]"}, on assignment with one of our direct clients, ${client}, in the role of ${role}.`;
  }

  if (engagement === "ic") {
    const first = firstNameOf(employee);
    return (
      `This letter is to confirm that ${employee} is working for one of our direct clients, ${client}, as a ${role}. ` +
      `${first} has been working with ${company.name} as an Independent Contractor through ${contractingEntity || "[Contracting Entity]"}.`
    );
  }

  // W2
  let sentence = `This letter is to confirm that ${employee} is employed by ${company.name} as a ${employmentType} employee on assignment with one of our direct clients, ${client}, in the role of ${role}`;
  if (workLocation) sentence += ` at ${workLocation}`;
  return sentence + ".";
}
...
function updateFieldVisibility() {
  ...
  workLocationField.hidden = engagement !== "w2";
...
function updatePreview() {
  ...
  previewMastheadTagline.textContent = companyTaglineEl.value.trim();
  ...
  if (benefitsText) {
    bodyHtml += `<p class="sheet-section-title">Benefits</p><p>${benefitsText}</p>`;
  }
...
companyTaglineEl.value = DEFAULT_TAGLINE;
